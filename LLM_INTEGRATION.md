...
```javascript
// 积极模式（未越网）
const positivePatterns = ["我感到", "我觉得", "我观察到", "我希望", "我需要", "我认为"];

// 消极模式（越网）
const negativePatterns = ["你总是", "你从不", "你就是", "你应该", "你必须"];
```
...
interface AnalysisResult {
  isCrossNet: "是" | "否";
  senderState: string; // 发送者的需求、动机、意图和背景分析
  receiverImpact: string; // 对接收者的预期影响、感受和反应
  evidence: string; // 具体的越网证据或未越网的证明
